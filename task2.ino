...
// Thresholds according to prompt
const int LIGHT_THRESHOLD = 505;            // Below half = Storm
const float DIST_THRESHOLD = 100.0;         // Below 100cm = Charybdis
const unsigned long DANGER_LIMIT_MS = 5000; // 5s continuous danger = Wrecked
...
void loop() {
  // If wrecked, hold state permanently until simulation is restarted
  ...
  if (currentState == STATE_ANCHOR_DROPPED) {
    ...
    return; // Completely immune to dangers while anchored
  }
